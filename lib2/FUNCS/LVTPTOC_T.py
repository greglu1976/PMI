# Ступень МТЗ трансформатора с НН (для исполнения Т)
...
#SGF5 - Реж_КПОН - Режим контроля от КПОН (Не предусмотрено/ Предусмотрено)
#SGF6 - Контр_СВ - Режим контроля СВ НН (Не предусмотрено/ Блокировка ступени при включенном СВ/ 	Блокировка ступени при отключенном СВ)

from lib2.TIMERS.TIMERS import TON  
from lib2.TRIGGERS.TRIGGERS import RSTrigger

class LVTPTOC:
    def __init__(self, SGF1, SGF2, SGF3, SGF4, SGF5, SGF6, T1, Iset, Icoarse):
        self.SGF1 = SGF1
        self.SGF2 = SGF2
        self.SGF3 = SGF3
        self.SGF4 = SGF4
    ...
    def Step(self, VYVOD, OV, OVst, NaSign, SV1vkl, IA, IB, IC, BNTpuskA, BNTpuskB, BNTpuskC, KZN1neipr, KPON1pusk, VNN1vkl):
        vvod, oper_vyvod = self.PrePreStep(VYVOD, OV, OVst)
        io_A, io_B, io_C, kpon_pusk, set_changer = self.PreStep(IA, IB, IC, KZN1neipr, KPON1pusk, VNN1vkl)
        mtzA_pusk, mtzB_pusk, mtzC_pusk, gen_pusk, mtz_srabsign, mtz_srab, ET = self.AfterStep(NaSign, SV1vkl, io_A, io_B, io_C, BNTpuskA, BNTpuskB, BNTpuskC, kpon_pusk, vvod)
    ...
    def AfterStep(self, NaSign, SV1vkl, io_A, io_B, io_C, BNTpuskA, BNTpuskB, BNTpuskC, kpon_pusk, vvod):
        ...
        sv_ctl = 0 if (self.SGF6==0) else (SV1vkl) if (self.SGF6==1) else not(SV1vkl) 
    ...
    def PreStep(self, IA, IB, IC, KZN1neipr, KPON1pusk, VNN1vkl): # Вспомогательный метод для предварительного вычисления значений для БНТ
        _p001 = (KZN1neipr if (self.SGF4==1) else 0) or (KPON1pusk and (not(KZN1neipr if(self.SGF4==2) else 0)))
        kpon_gen1 = 0 if (self.SGF5==0) else (VNN1vkl and _p001)
        #kpon_gen2 = 0 if (self.SGF6==0) else (VNN2vkl and (KZN2neipr if (self.SGF4==1) else 0) or (KPON2pusk and not(KZN2neipr if(self.SGF4==2) else 0)))
        kpon_gen = ((1 if (self.SGF5==0) else 0) or not(VNN1vkl))

        #print('kpon_gen', self.SGF5)
        kpon_pusk = kpon_gen1 or kpon_gen

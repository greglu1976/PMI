...
class SWITCH:
    ...
    OV_tsd, 
    pusk_urov_vnesh,
    vnesh_otkl_zdz, vnesh_otkl_urov
    ):
        
        # === НАЧАЛО ИЗМЕНЕНИЙ ===
        # Жесткая логическая связь: Дистанционное = НЕ Местное
        # Если mestnoe=1, то distanz станет 0. Если mestnoe=0, то distanz станет 1.
        # Мы перезаписываем входящий аргумент distanz перед использованием.
        distanz = 1 - int(mestnoe) 
        # === КОНЕЦ ИЗМЕНЕНИЙ ===


        vvod_hvcbptrc1_hvtcboff, oper_vyvod_hvcbptrc1_hvtcboff, otkl_hvcbptrc1_hvtcboff, otkl_avar_hvcbptrc1_hvtcboff = self.hvtcboff.Step(VYVOD, OV_hvcbptrc1_hvtcboff=0, LO_t_srab=0, vnesh_otkl_zdz=vnesh_otkl_zdz, vnesh_otkl_urov=vnesh_otkl_urov)
        ...
        pusk_t_lvalh = self.t_lvalh.Step(VYVOD=VYVOD, sign_ps_tuple=(v_samoproisv_otkl_rcbf1_lvcbsup, neispr_V_rcbf1_lvcbsup, self.srab_na_sebya_rbrf1_tpbrf, v_avar_otkl_rcbf1_lvcbsup), sgf13_sign=ss_prev_vrem_per_ka)

...
# тест GEMINI не работают подсказки
...
# Импортируем вашу библиотеку (как было в оригинале)
from lib2.PARTS.MTZ_T import partOfFsuInTOC

# Импорты для работы с JSON (из образца)
try:
    from MainConfigHandler import MainConfigHandler
    from SettingsHandler import SettingsHandler
except ImportError:
    print("WARNING: MainConfigHandler or SettingsHandler not found. JSON features may not work.")
    MainConfigHandler = None
...
class PartOfFsuInTOC_GUI:
    def __init__(self, root):
        ...
        # === ЗАГРУЗКА МЕТАДАННЫХ (НОВЫЙ ФУНКЦИОНАЛ) ===
        try:
            if MainConfigHandler:
                self.meta_handler = MainConfigHandler.from_json_file("meta.json")
            else:
                self.meta_handler = None
        except Exception as e:
            print(f"?? Не удалось загрузить meta.json: {e}")
            self.meta_handler = None
    ...
    def create_widgets(self):
        ...
        for key, var in self.sgf_params.items():
            ...
            elif key=="LVTTOC_1_RBLC1_StepSel" or key=="T_LVARCTOC_1_PTOC1_StrMod":
        ...
        # Button Load JSON (НОВАЯ КНОПКА)
        ttk.Button(buttons_frame, text="Load JSON", command=self.load_settings_from_json).grid(row=0, column=5, padx=2, pady=10)

        # Button Save JSON (НОВАЯ КНОПКА)
        ttk.Button(buttons_frame, text="Save JSON", command=self.save_settings_to_json).grid(row=0, column=6, padx=2, pady=10)

        # Поля для задания имени файла (СДВИНУТЫ СТОЛБЦЫ)
    ...
    # === МЕТОДЫ ДЛЯ РАБОТЫ С JSON ФАЙЛАМИ УСТАВОК (НОВЫЙ ФУНКЦИОНАЛ) ===

    def load_settings_from_json(self):
        """Загружает SGF и T-параметры из JSON-файла с суффиксом _SG1."""
        if not SettingsHandler:
            messagebox.showerror("Ошибка", "Модуль SettingsHandler не найден.")
            return
        ...
        try:
            ...
            print("? Параметры обновлены из JSON")
        except Exception as e:
    ...
    def save_settings_to_json(self):
        ...
        file_path = askopenfilename(title="Сохранить уставки как...", filetypes=[("JSON files", "*.json")], defaultextension=".json")
        if not file_path:
            return
        try:
            try:
            ...
            for base_key in self.sgf_params:
                ...
                try:
                    ...
                    numeric_value = int(numeric_value) if numeric_value.is_integer() else numeric_value
                except (ValueError, TypeError):
                    numeric_value = 0

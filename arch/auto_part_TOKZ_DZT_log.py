...
class PartOfTOKGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Тестирование токовых функций ФСУ ДЗТ2, v0.1 от 15.05.25 с логгером")
        self.part = None
        ...
        self.sgf_params = {
            ...
            "SGF1_lvoileqpalc_eqpalc": tk.IntVar(value=0),
            "SGF2_lvoileqpalc_eqpalc": tk.IntVar(value=0),
            "SGF3_lvoileqpalc_eqpalc": tk.IntVar(value=0),
        ...
        # Настройка логгера после создания console_output
        gui_handler = GUILogHandler(self.console_output)
    ...
    def create_widgets(self):
        ...
        console_frame = ttk.LabelFrame(self.root, text="Лог сообщений")
        console_frame.grid(row=4, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")

        self.console_output = ScrolledText(console_frame, height=10, wrap=tk.WORD)
        self.console_output.pack(fill=tk.BOTH, expand=True)
        self.console_output.config(state=tk.DISABLED)

        # Кнопка очистки лога
        clear_button = ttk.Button(console_frame, text="Очистить лог", command=self.clear_log)
    ...
    def init_part(self):
        self.part = partTOKZ(
            ...
            SGF1_lvoileqpalc_eqpalc=self.sgf_params["SGF1_lvoileqpalc_eqpalc"].get(),
            SGF2_lvoileqpalc_eqpalc=self.sgf_params["SGF2_lvoileqpalc_eqpalc"].get(),
            SGF3_lvoileqpalc_eqpalc=self.sgf_params["SGF3_lvoileqpalc_eqpalc"].get(),
            ...
            T1_lvoileqpalc_eqpalc=self.settings["T1_lvoileqpalc_eqpalc"].get(),
        ...
        self.logger.info("partTOKZ initialized")

    def start_polling(self):
        if self.part is None:
            self.logger.warning("partTOKZ not initialized")
            return
    ...
    def save_to_excel(self):
        ...
        if not function or not mode:
            self.logger.warning("Поля 'Функция' и 'Режим' должны быть заполнены")
            return
        ...
        wb.save(output_file)
        self.logger.info(f"Data saved to {output_file} with formatting")

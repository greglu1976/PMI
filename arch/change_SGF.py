...
def replace_values_in_xlsx(file_path, replacement_rules, sheet_name="SGF_Parameters"):
    ...
    if modifications_made:
        # Сохраняем изменения в новый файл (чтобы не перезаписывать оригинал)
        output_path = Path(file_path)
        new_path = output_path.with_stem(output_path.stem + "_modified")
        wb.save(new_path)
        print(f"  ✅ Файл сохранён: {new_path.name}")
    else:
        print(f"  ℹ️  В файле {file_path.name} не было изменений")
    
    wb.close()
    return new_path if modifications_made else None
...
def process_all_xlsx_files(replacement_rules, sheet_name="SGF_Parameters"):
    ...
    if not xlsx_files:
        print("❌ В текущей папке не найдено xlsx-файлов")
        return

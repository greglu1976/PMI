# Меняет обозначения столбцов

import os
import pandas as pd

# Корневая папка
root_path = 'pmi_tokdzt'
                ...
                with pd.ExcelFile(file_path) as xls:
                    ...
                    for sheet_name in ['SGF_Parameters', 'Settings']:
                        if sheet_name in xls.sheet_names:
                            ...
                            # Суффиксы для поиска
                            target_suffixes = ['SGF1', '_ptoc1', '_ptoc2', '_ptoc3', '_ptuv1', '_phar1', '_rblc1']
                            target_columns = [col for col in df.columns 
                                            if any(col.endswith(suffix) for suffix in target_suffixes)]
                            
                            if target_columns:
                                # Переименование столбцов
                                renamed_columns = {col: col + '_lvttoc' for col in target_columns}

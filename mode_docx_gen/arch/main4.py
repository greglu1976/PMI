# В этой версии изменен подход, то есть сначала генерируются все json
# потом вставляются последовательно в документ
        ...
        for fbname, functions in general_data.items():
            ...
            for func_name, switches in functions.items():
                if func_name == "":  # Если ключ пустой
                    ...
                    #aragraph = doc.add_heading(f"{func_desc} ({func_short_name})", level=4)
...
if __name__ == "__main__":
    ...
    doc.add_paragraph('Выдаваемые сигналы', style='ЮИ_Таблица_Название')
    ...
    doc.add_paragraph('Контролируемые сигналы', style='ЮИ_Таблица_Название')

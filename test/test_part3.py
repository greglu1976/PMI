...
class PartOfSwitchGUI:
    ...
    def create_widgets(self):
        ...
        self.input_frame.grid(row=3, column=0, padx=10, pady=10, sticky="w")

        # Кнопка для переключения режимов входных данных
        self.input_mode_button = ttk.Button(self.input_frame, text="Режим 1", command=self.toggle_input_mode)
        self.input_mode_button.grid(row=0, column=0, pady=5)

        # Панель для входных данных
        self.input_fields = {}
    ...
    def create_input_fields(self):
        """Создает поля ввода для текущего режима."""
        for widget in self.input_frame.winfo_children():
            if isinstance(widget, ttk.Checkbutton) or isinstance(widget, ttk.Entry):
                widget.destroy()

        # Выбираем текущий набор входных переменных
        current_input_vars = self.input_vars_mode1 if self.current_mode == 1 else self.input_vars_mode2

        row = 1
        col = 0
        for key, var in current_input_vars.items():
            if isinstance(var, tk.IntVar):
                ttk.Checkbutton(self.input_frame, text=key, variable=var).grid(row=row, column=col, sticky="w")
            elif isinstance(var, tk.DoubleVar):
                ttk.Label(self.input_frame, text=key).grid(row=row, column=col, sticky="w")
    ...
    def toggle_input_mode(self):
        """Переключение между режимами входных данных."""
        if self.current_mode == 1:
            self.current_mode = 2
            self.input_mode_button.config(text="Режим 2")
        else:
            self.current_mode = 1
            self.input_mode_button.config(text="Режим 1")
        self.create_input_fields()
    ...
    def poll_inputs(self):
        while self.is_polling:
            ...
            inputs = (
                ...
                if self.current_mode == 1
                else {key: var.get() for key, var in self.input_vars_mode2.items()}
            )
    ...
    def save_to_excel(self):
        ...
        with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
            ...
            inputs_mode1_df.to_excel(writer, sheet_name="Inputs", index=False)
            inputs_mode2_df.to_excel(writer, sheet_name="Inputs", startrow=len(inputs_mode1_df) + 2, header=False, index=False)
            outputs_mode1_df.to_excel(writer, sheet_name="Fixed_Outputs", index=False)
            outputs_mode2_df.to_excel(writer, sheet_name="Fixed_Outputs", startrow=len(outputs_mode1_df), header=False, index=False)
        ...
        format_sheet(wb["Inputs"], pd.concat([inputs_mode1_df, inputs_mode2_df]))

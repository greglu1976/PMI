...
class PartOfSwitchGUI:
    ...
    def create_widgets(self):
        ...
        self.mode_button.grid(row=0, column=9, padx=5, pady=5)
        ...
        self.status_label.grid(row=0, column=10, padx=5, pady=5)
    ...
    def start_polling(self):
        ...
        # Запускаем таймер для автоматического переключения режимов
        self.start_mode_timer()

    def start_mode_timer(self):
        """Запускает таймер для автоматического переключения режимов."""
        if self.mode_timer:
            self.root.after_cancel(self.mode_timer)
        self.mode_timer = self.root.after(2000, self.auto_switch_mode)  # 2000 мс = 2 секунды

    def auto_switch_mode(self):
        """Автоматическое переключение режимов."""
        if self.is_polling:
            self.toggle_mode()
            self.start_mode_timer()  # Перезапускаем таймер

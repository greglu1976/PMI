# (T_JNTRBRE) ЗАПВ - Логика запрета АПВ (в составе устройства ЮНИТ-М3-Т)
# SGF1 - Ввод_функции - Ввод функции в работу (Не предусмотрено/ Предусмотренно)
# SGF2 - ЗАПВ_МТЗ2 - Запрет АПВ при срабатывании МТЗ 2 ст. (Не предусмотрено/ Предусмотрено)
# SGF3 - ЗАПВ_МТЗ3 - Запрет АПВ при срабатывании МТЗ 3 ст. (Не предусмотрено/ Предусмотрено)
...
class T_JNTRBRE:
    ...
    def Step(self, VYVOD, OV, OVzapv, pusk, mtz2_srab, mtz3_srab):
        vvod = (not(OV or OVzapv or VYVOD)) and (self.SGF1==1) # МТЗ: Ввод
        oper_vyvod = (OV or OVzapv or VYVOD) and (self.SGF1==1) # МТЗ: Оперативный вывод
        mtz2_sr = 0 if (self.SGF2==0) else mtz2_srab
        mtz3_sr = 0 if (self.SGF3==0) else mtz3_srab        
        zapret = vvod and (pusk or mtz2_sr or mtz3_sr)
...
if __name__ == "__main__":
    ps = T_JNTRBRE(SGF1=1, SGF2=0, SGF3=1)
    res = ps.Step(0,0,0,0,0,1)
    print(res)